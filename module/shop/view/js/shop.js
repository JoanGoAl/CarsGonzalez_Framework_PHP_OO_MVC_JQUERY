...
function load_details(id) {
    ...
    ajaxPromise("POST", url, "json", { idcar: id })
        .then(function(json) {
            ...
            catTitle.appendChild(document.createTextNode(json[0][0].name_brand + ' ' + json[0][0].name_model))

            // console.log(json);
            catDesc.appendChild(document.createTextNode('CV: ' + json[0][0].cv + ' Kilometros: ' + json[0][0].kms + ' Numero de puertas: ' + json[0][0].num_door))
            catPrice.appendChild(document.createTextNode('Precio: ' + json[0][0].price + '€'))
...
function load_filters() {
    ...
    // Filtrado por marcas
    url = friendlyURL("?page=shop&op=brands_and_model")
...
function ajaxForSearch(pagination) {
    ...
    ajaxPromise("POST", url, "json", totalData)
        .then((filterCars) => {
...
function load_items(filterCars) {
    ...
    if (filterCars) {
        ...
        url = friendlyURL("?page=shop&op=list_cars")
        ajaxPromise("GET", url, "json")
            .then(function(jsonCars) {
